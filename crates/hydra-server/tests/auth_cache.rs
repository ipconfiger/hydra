...
#[test]
fn cache_decision_delegates_to_pure() {
    let (time, clock) = frozen_clock();
    let cache = AuthCache::with_clock(ALLOW_TTL, DENY_TTL, clock);

    // allow entry
    cache.set("t1", "sk-allow", true, Duration::from_secs(60));
    // deny entry
    cache.set("t1", "sk-deny", false, Duration::from_secs(60));
    ...
    assert_eq!(cache.check("t1", "sk-allow"), pure_allow);
    assert_eq!(cache.check("t1", "sk-allow"), Verdict::Hit(true));
    assert_eq!(cache.check("t1", "sk-deny"), pure_deny);
    assert_eq!(cache.check("t1", "sk-deny"), Verdict::Hit(false));

    // unknown key / tenant → Miss (pure agrees)
    assert_eq!(cache.check("t1", "sk-unknown"), cache_decision(None, now));
    assert_eq!(cache.check("t2", "sk-allow"), Verdict::Miss);
}

#[test]
fn cache_decision_expired_is_miss_like_pure() {
    let (time, clock) = frozen_clock();
    let cache = AuthCache::with_clock(ALLOW_TTL, DENY_TTL, clock);
    cache.set("t1", "sk-x", true, Duration::from_secs(60));
...
fn cache_concurrent_writes_all_visible() {
    ...
    std::thread::scope(|s| {
        for i in 0..n {
            ...
            s.spawn(move || {
                cache.set("t1", &format!("key-{i}"), true, Duration::from_secs(60));
            });
        }
    });
    assert_eq!(cache.len(), n as usize);
    for i in 0..n {
        assert_eq!(cache.check("t1", &format!("key-{i}")), Verdict::Hit(true));
    }
}
...
#[test]
fn cache_set_and_invalidate_keys() {
    let cache = AuthCache::new(ALLOW_TTL, DENY_TTL);
    cache.set("t1", "k1", true, Duration::from_secs(60));
    cache.set("t1", "k2", true, Duration::from_secs(60));
    cache.set("t1", "k3", true, Duration::from_secs(60));
    ...
    assert_eq!(removed, 2);
    assert_eq!(cache.check("t1", "k1"), Verdict::Miss);
    assert_eq!(cache.check("t1", "k2"), Verdict::Miss);
    // k3 untouched
    assert_eq!(cache.check("t1", "k3"), Verdict::Hit(true));
    assert_eq!(cache.len(), 1);
}

#[test]
fn cache_invalidate_keys_is_tenant_scoped() {
    // same api-key under two tenants: invalidating one tenant must NOT touch
    // the other (the cache key is (tenant_id, sha256)).
    let cache = AuthCache::new(ALLOW_TTL, DENY_TTL);
    cache.set("t1", "shared", true, Duration::from_secs(60));
    cache.set("t2", "shared", true, Duration::from_secs(60));

    let removed = cache.invalidate("t1", &["shared".to_string()]);
    assert_eq!(removed, 1);
    assert_eq!(cache.check("t1", "shared"), Verdict::Miss);
    assert_eq!(cache.check("t2", "shared"), Verdict::Hit(true));
}
...
#[test]
fn cache_invalidate_tenant() {
    let cache = AuthCache::new(ALLOW_TTL, DENY_TTL);
    cache.set("t1", "k1", true, Duration::from_secs(60));
    cache.set("t1", "k2", false, Duration::from_secs(60));
    cache.set("t2", "k3", true, Duration::from_secs(60));
    ...
    assert_eq!(cache.check("t1", "k2"), Verdict::Miss);
    // t2 untouched
    assert_eq!(cache.check("t2", "k3"), Verdict::Hit(true));
    assert_eq!(cache.len(), 1);
}

#[test]
fn cache_invalidate_tenant_missing_is_zero() {
    let cache = AuthCache::new(ALLOW_TTL, DENY_TTL);
    assert_eq!(cache.invalidate_tenant("ghost"), 0);
}
...
#[test]
fn cache_gc_evicts_expired() {
    let (time, clock) = frozen_clock();
    let cache = AuthCache::with_clock(ALLOW_TTL, DENY_TTL, clock);

    cache.set("t1", "fresh", true, Duration::from_secs(60));
    cache.set("t1", "old1", false, Duration::from_secs(60));
    cache.set("t2", "old2", true, Duration::from_secs(60));
...
#[test]
fn cache_gc_keeps_live_entries() {
    let (time, clock) = frozen_clock();
    let cache = AuthCache::with_clock(ALLOW_TTL, DENY_TTL, clock);

    cache.set("t1", "live", true, Duration::from_secs(120));
    cache.set("t1", "dead", true, Duration::from_secs(30));

    // advance 60s: dead (30s TTL) expired, live (120s TTL) still valid
    *time.lock().unwrap() += Duration::from_secs(60);
    let evicted = cache.gc();
    assert_eq!(evicted, 1);
    assert_eq!(cache.check("t1", "live"), Verdict::Hit(true));
    assert_eq!(cache.check("t1", "dead"), Verdict::Miss);
    assert_eq!(cache.len(), 1);
}

#[test]
fn cache_gc_on_empty_is_zero() {
...
#[test]
fn cache_key_is_sha256_not_plaintext() {
    let cache = AuthCache::new(ALLOW_TTL, DENY_TTL);
    let secret = "sk-super-secret-api-key-12345";
    cache.set("t1", secret, true, Duration::from_secs(60));
...
#[test]
fn cache_set_then_invalidate_then_set_works() {
    // round-trip: set → check → invalidate → miss → set again
    let cache = AuthCache::new(ALLOW_TTL, DENY_TTL);
    cache.set("t1", "k", true, Duration::from_secs(60));
    assert_eq!(cache.check("t1", "k"), Verdict::Hit(true));
    assert_eq!(cache.invalidate("t1", &["k".to_string()]), 1);
    assert_eq!(cache.check("t1", "k"), Verdict::Miss);
    cache.set("t1", "k", false, Duration::from_secs(60));
    assert_eq!(cache.check("t1", "k"), Verdict::Hit(false));
}
